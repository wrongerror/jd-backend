...
remove_debian_systen_maint_password() {
  ...
  #
  sed 's/password = .*/password = 123456 /g' -i /etc/mysql/debian.cnf
}
...
initialize_mysql_database() {
  ...
  if [ ! -d ${MYSQL_DATA_DIR}/mysql ]; then
    ...
    mysql -uroot -e "GRANT ALL PRIVILEGES on *.* TO 'debian-sys-maint'@'localhost' IDENTIFIED BY '123456' WITH GRANT OPTION;"
...
create_users_and_databases() {
  ...
  if [ -n "${DB_USER}" -o -n "${DB_NAME}" ]; then
    ...
    # migrate migrations
    echo "django migrate migrations"
    cd /var/www/hawkapi
    . venv/bin/activate
